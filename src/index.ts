...
server.tool(
  ...
  'Get one or more tags by ID(s) - accepts single ID or array of IDs for batch operations',
  {
    id: z.union([
      ...
      z
        .array(z.string().regex(/tag-[\d]+/, 'Invalid tag ID'))
...
server.tool(
  ...
  async ({ username, blockedTodoId, blockerTodoId }) => {
    const result = await safeExecute(async () => {
      ...
      const removed = await todoService.removeBlockerDependency(
        blockedTodoId,
        blockerTodoId,
        username
      );
 ...
 */
server.tool('list-users', 'List all registered users', {}, async () => {
  const result = await safeExecute(async () => {
    ...
    const userList = users
      .map(
        (user) => `- ${user.username} (created: ${new Date(user.createdAt).toLocaleDateString()})`
      )
      .join('\n');
